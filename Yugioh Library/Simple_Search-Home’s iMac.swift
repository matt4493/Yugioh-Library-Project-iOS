...
class Simple_Search : UIViewController , UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate {
    ...
    @IBAction func selectedSeries(sender: UIButton) {
        ...
        self.presentViewController(alertSeriesController, animated: true, completion: nil)
    }
    
    @IBAction func selectedSet(sender: UIButton) {
    ...
    self.setup_boosteralertbox_based_on_series_chosen((self.selectedSeriesO.titleLabel?.text)!)
        ...
        self.presentViewController(alertBoosterController, animated: true, completion: nil)
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell
    {
        ...
        CardName.sharedInstance.cardName = card_name
        ...
        Simple_Search.process_AND_return_card_URLS(CardName.sharedInstance.cardName!) {
            returnedURL in
            self.card_image_url = returnedURL
            
            //print("FUNC = \(card_image_url)")
            var check_apost = self.card_image_url.stringByReplacingOccurrencesOfString("%27", withString: "'")
            let check_special = check_apost.stringByReplacingOccurrencesOfString("", withString: "")
            ...
            let finished_URL = NSURL(string: check_apost)
            
            cell.card_imageIV.kf_showIndicatorWhenLoading = true
            
            cell.card_imageIV.kf_cancelDownloadTask()
            
            //cell.card_imageIV.kf_setImageWithURL(finished_URL!)
            
            cell.card_imageIV.kf_setImageWithURL(finished_URL!, placeholderImage: nil,
                                                  optionsInfo: [.Transition(ImageTransition.Fade(1))],
                                                completionHandler: { image, error, cacheType, imageURL in
                                                    print("\(card_name)): Finished")
            })
            
            // Use Haneke to fetch images from Cache or Network & set them to Cell
        ...
        if card_type.containsString("Synchro") == true {
            cell.backgroundColor = hexStringToUIColor("#CCCCCC")
            //print("Synchro COLOR")
        }else if card_type.containsString("Fusion") == true {
            cell.backgroundColor = hexStringToUIColor("#a086b7")
            //print("Fusion COLOR")
        }
        else if card_type.containsString("Xyz") == true {
            cell.backgroundColor = hexStringToUIColor("#FFFFFF")
           // print("Xyz COLOR")
        }
        else if card_type.containsString("Ritual Spell Card") == true {
        ...
        else if card_type.containsString("Ritual") == true {
            cell.backgroundColor = hexStringToUIColor("#9DB5CC")
           // print("Ritual COLOR")
        }
        else if card_type.containsString("Spell Card") == true {
            cell.backgroundColor = hexStringToUIColor("#1d9e74")
            //print("Spell Card COLOR")
        }
        else if card_type.containsString("Trap Card") == true {
            cell.backgroundColor = hexStringToUIColor("#BC5A84")
            //print("Trap Card COLOR")
        }
        else if card_type.containsString("Effect"){
    ...
    func construct_image_url(var passed_SetInitial: String, passed_SetNumber: String) -> String{
        ...
        if passed_SetInitial.lowercaseString.characters.contains(searchCharacter) {
    ...
    override func viewDidLoad() {
        ...
        card_nameEntry.addTarget(self, action: "textFieldDidChange:", forControlEvents: UIControlEvents.EditingChanged)
    ...
    func retrieve_selectedSeries(chosen_series: String) {
        
        if chosen_series == "Cancel" {
            
            }
        else{
            selectedSeriesO.setTitle(chosen_series, forState: UIControlState.Normal)
            selected_series = chosen_series
        }
    }
    
    func retrieve_selectedSet(chosen_set: String) {
        ...
        if chosen_set == "Cancel"{
            ...
            selectedSetO.setTitle(chosen_set, forState: UIControlState.Normal)
            selected_set = chosen_set
        }
    }
    
    // Determine the user options selected to search for the correct card results
    func determine_search_conditions(enter_card_name: String, chosen_series: String, chosen_set: String) {
    ...
    func setup_boosteralertbox_based_on_series_chosen(series_chosen: String) {
    ...
    func CollectDetails(var passedCardName: String){
        // Collect Card Details
        
        CardName.sharedInstance.cardName = CardAdapter_Object.getCardName(passedCardName)
        Type.sharedInstance.returnedType = CardAdapter_Object.getType(passedCardName)
        Attribute.sharedInstance.returnedAttribute = CardAdapter_Object.getAttribute(passedCardName)
        SummonRequirements.sharedInstance.returnedSummonRequirements = CardAdapter_Object.getSummonRequirements(passedCardName)
        Description.sharedInstance.returnedDescription = CardAdapter_Object.getDescription(passedCardName)
        SpellSpeed.sharedInstance.returnedSpellSpeed = CardAdapter_Object.getSpellSpeed(passedCardName)
        Level.sharedInstance.returnedLevel = CardAdapter_Object.getLevel(passedCardName)
        Rank.sharedInstance.returnedRank = CardAdapter_Object.getRank(passedCardName)
        Atk.sharedInstance.returnedAtk = CardAdapter_Object.getAtk(passedCardName)
        Def.sharedInstance.returnedDef = CardAdapter_Object.getDef(passedCardName)
        Setini.sharedInstance.returnedSetini = CardAdapter_Object.getSetini(passedCardName)
        SetNumber.sharedInstance.returnedSetNumber = CardAdapter_Object.getSetNumber(passedCardName)
        CardPass.sharedInstance.returnedCardPass = CardAdapter_Object.getCardPass(passedCardName)
        Rarity.sharedInstance.returnedRarity = CardAdapter_Object.getRarity(passedCardName)
        Legality.sharedInstance.returnedLegality = CardAdapter_Object.getLegality(passedCardName)
        
        var check_CardName = CardName.sharedInstance.cardName
    ...
    func Check_CardDetails_Segue(passedTRUEFALSE_Value: Int) {
    ...
    class func process_AND_return_card_URLS(passed_CardName: String, completionHandler: (returnedURL: String) -> ()) {
        
        let replace_spaces = passed_CardName.stringByReplacingOccurrencesOfString(" ", withString: "+", options: NSStringCompareOptions.LiteralSearch, range: nil)
        //let replace_special_characters = replace_spaces.stringByReplacingOccurrencesOfString("%C3%9C", withString: "Ü")
        if passed_CardName.containsString("Beast Machine King"){
            completionHandler(returnedURL: "http://static.api3.studiobebop.net/ygo_data/card_images/Beast_Machine_King_Barbaros_Ür.jpg")
        }else {
            let url = NSURL(string: "http://yugiohprices.com/api/card_image/" + "\(replace_spaces)")
